    ...
    async def initiate_call_for_task(self, task: Task) -> bool:
        ...
        try:
            ...
            await db_manager.update_task_status(
                task_id=task.id, 
                status=TaskStatus.INITIATING_CALL
            )
            ...
            if call_record and call_record.id:
                 ...
                 # Directly await db_manager.update_call_status as it's async def
                 await db_manager.update_call_status(
                     call_id=call_record.id, # This was call_id, assuming it's for a Call object
                     status=CallStatus.FAILED_INTERNAL_ERROR, 
                     hangup_cause="Initiation process failure" # This matches update_call_status signature
                 )
